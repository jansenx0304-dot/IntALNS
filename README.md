...
| `results/` | 90 份原始完整记录、90 份运行状态、版本来源与 SHA-256 校验 |
| `scripts/` | 从归档结果生成当前论文图表，以及干净编译论文 |

`agent` 是 IntALNS 的原始方法标识。Example 库中的版本标识属于正式模型输入，保留实际执行内容；四份库按规模和搜索状态检索，并非四套候选算法。
...
本包用于复现既定方法和现有论文结果，不执行任何参数选择。归档实例所在目录沿用原始名称 `test`；来源记录将当前结果界定为固定 6800–6804 实例集上的回顾性评估，不能把它理解为新增且从未查看过的独立盲测。90 份记录中，35 份 Agent 记录保留 V15 标签并按已有输入等价核验复用，10 份 T300 M10/M50 Agent 记录为 V17，45 份 Basic 记录保持原样。`results/provenance.json` 保留逐记录哈希及对应关系。归档完整保留全部 45 对条件。
